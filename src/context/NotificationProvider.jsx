...
export default function NotificationProvider({ children }) {
  ...
  //to change the notification we use hook
  const [notification, setNotification] = useState("");

  //to change the class which defines the bg color we use useState hook
  const [classes, setClasses] = useState("");

  const updateNotification = (type, value) => {
    if (timeoutId) clearTimeout(timeoutId);
    
    //according to 'type' (error, success and warning) background color will be decided
    switch (type) {
      case "error":
